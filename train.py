...
# This class represents a attention decoder used for transliteration
class AttentionDecoder(nn.Module):
    def __init__(self, embeddingSize, noOfLayer, hiddenSize, cellType, dropout, inputVocabularySize, outputVocabularySize):
        super(AttentionDecoder, self).__init__()
        self.attention = nn.Linear(hiddenSize, hiddenSize, bias=False)
        self.dropout = nn.Dropout(dropout)
        self.embedding = nn.Embedding(inputVocabularySize, embeddingSize)
        self.cell = getattr(nn, cellType)(embeddingSize + hiddenSize, hiddenSize, noOfLayer, dropout=dropout)
        self.fc = nn.Linear(hiddenSize*2, outputVocabularySize)
    
    # Defiining forward propagation through the decoder
    def forward(self, x, encoderOutput, hidden, cellState):
        embeddedRep = self.dropout(self.embedding(x.unsqueeze(0)))
        
        # Computing the attention scores, apply softmax to get attention weights,
        # and calculating the context vector using these weights and the encoder output.
        attnScore = (self.attention(encoderOutput).permute(1, 0, 2) @ hidden[-1:].permute(1, 2, 0)).permute(2, 0, 1)
        attnWeight = nn.functional.softmax(attnScore, dim=2)
        context = (attnWeight.permute(1, 0, 2) @ encoderOutput.permute(1, 0, 2)).permute(1, 0, 2)
        
        if isinstance(self.cell, nn.LSTM):
            outputs, (hidden, cellState) = self.cell(torch.cat([embeddedRep, context], dim=2), (hidden, cellState))
        else:
            outputs, hidden = self.cell(torch.cat([embeddedRep, context], dim=2), hidden)

        outputs = self.fc(torch.cat([outputs, context], dim=2)).squeeze(0)

        return outputs, hidden, cellState, attnWeight.squeeze(0)


# This class represents a RNN model that can do transliteration
class TransliterationModel(nn.Module):
    def __init__(self, embeddingSize, noOfEncoderLayer, noOfDecoderLayer, hiddenSize, cellType, dropout, inputVocabularySize, outputVocabularySize, attention):
        ...
        self.encoder = Encoder(embeddingSize, noOfEncoderLayer, hiddenSize, cellType, dropout, inputVocabularySize).to(device)
        if self.attention == 'Yes':
            self.decoder = AttentionDecoder(embeddingSize, noOfDecoderLayer, hiddenSize, cellType, dropout, outputVocabularySize, outputVocabularySize).to(device)
        else:    
            self.decoder = Decoder(embeddingSize, noOfDecoderLayer, hiddenSize, cellType, dropout, outputVocabularySize, outputVocabularySize).to(device)
    ...
    def forward(self, x, y, teacher_force_ratio = 0.5):
        ...
        attnMatrix = torch.zeros(y.shape[0], x.shape[1], x.shape[0]).to(device)

        # passing the input through encoder
        encoderOutput, hidden, cell = self.encoder(x)
        ...
        for i in range(1, y.shape[0]):
            if self.attention == 'Yes':
                output, hidden, cell, attnWeight = self.decoder(decoderInput, encoderOutput, hidden, cell)
                attnMatrix[i] = attnWeight
            else:
    ...
    def trainModel(self, train_dataloader, val_dataloader, noOfEpochs):
        for epoch in range(noOfEpochs):
            ...
            for batch_idx, (x, y) in enumerate(train_dataloader):
                # forward propagation
                if self.attention == 'Yes':
                    y_hat, attnMatrix = self(x.T.to(device), y.T.to(device))
                else:
    ...
    def evaluateModel(self, dataloader):
        ...
        with torch.no_grad():
            for batch_idx, (x, y) in enumerate(dataloader):
                ...
                if self.attention == 'Yes':
                    y_hat, attnMatrix = self(x.T.to(device), y.T.to(device), teacher_force_ratio=0.0)
                else:
...
def main():
    # loading dataset
    train_dataset = pd.read_csv(trainDatasetPath, sep=',', header=None).values
    val_dataset = pd.read_csv(validationDatasetPath, sep=',', header=None).values
    test_dataset = pd.read_csv(testDatasetPath, sep=',', header=None).values
    ...
    test_dataloader = DataLoader(TensorDataset(x_test, y_test), batch_size=batchSize, shuffle=False)
    
    # Creating the model
    model = TransliterationModel(embeddingSize, noOfEncoderLayer, noOfDecoderLayer, hiddenSize, cellType, dropout, englishVocabulary.n_chars, bengaliVocabulary.n_chars, attention).to(device)
...
parser.add_argument("--train_dataset_path", "-trainPath", type=str, default="aksharantar_sampled/ben/ben_train.csv")
parser.add_argument("--validation_dataset_path", "-vaildPath", type=str, default="aksharantar_sampled/ben/ben_valid.csv")
parser.add_argument("--test_dataset_path", "-testPath", type=str, default="aksharantar_sampled/ben/ben_test.csv")
...
parser.add_argument("--encoder_layer", "-el", type=int, default=3)
parser.add_argument("--decoder_layer", "-dl", type=int, default=3)
parser.add_argument("--dropout", "-dp", type=float, default=0.2)
parser.add_argument("--bidirectional", "-bd", type=str, default='Yes')
parser.add_argument("--attention", "-atn", type=str, default='Yes')
args = parser.parse_args()

# Setting dataset path form command line arguments
trainDatasetPath = args.train_dataset_path
validationDatasetPath = args.validation_dataset_path
...
noOfEncoderLayer = args.encoder_layer
noOfDecoderLayer = args.decoder_layer
