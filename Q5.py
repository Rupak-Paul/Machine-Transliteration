...
def main():
    ...
    sweep_config = {
        ...
        'parameters': {
            ...
            'cellType': {'values':['LSTM', 'GRU']},
            'embeddingSize': {'values':[64, 128, 256]},
            'hiddenSize': {'values':[64, 128, 256]},
            'noOfEncoderLayer': {'values':[2, 3]},
            'noOfDecoderLayer': {'values':[2, 3]},
    ...
    sweep_id = wandb.sweep(sweep=sweep_config, project='CS23M056_DL_Assignment_3_Q5')
    wandb.agent(sweep_id, function=callMe, count=50)
    wandb.finish()


def callMe():
    wandb.init(project='CS23M056_DL_Assignment_3_Q5')
